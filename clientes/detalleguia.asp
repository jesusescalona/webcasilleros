...
'Brayan Ramirez - 02/12/2014 - Creacion de consulta para reemplazar la cookie de cordinadora
Set rsConfig = Server.CreateObject("ADODB.Recordset")
rsConfig.ActiveConnection = MM_CPV_STRING
rsConfig.Source = "SELECT isnull(CFG_COORDINADORA,0) as CFG_COORDINADORA from config"
          ...
          <td> <span class="btnAccion">Status</span><span class="titulos"><span class="txtTextoI"> <%=(DetMan.Fields.Item("crt_descripcion").Value)%></span> </span> 
            <table border="0" cellpadding="3" cellspacing="0" class="reciboSMALLCAP">
              ...
              <tr class="trs2"> 
                ...
                <td class="txtTexto"><%=(staguia.Fields.Item("descri").Value)%>
					<%' Enlace de rastreo coordinadora JUM-610-35344 - 23-01-2015 - Brayan Ramirez %>
				<%Select case cdbl(staguia.Fields.Item("codsta").Value)
					  
					  
					  	case cdbl(TRIM(rsConfig.Fields.Item("CFG_COORDINADORA")))  'REDESPACHO COORDINADORA %>
								 <span class="button">
								 <form id="frm_consultar_guia" action="http://www.coordinadora.com/portafolio-de-servicios/servicios-en-linea/rastrear-guias/" method="post" name="frm_consultar_guia" target="_blank">
								  <div class="frm_block">
								  <input type="hidden" name="coor_guia_home" id="coor_guia_home" class="required">
								  <input name="coor_guia" type="hidden" id="coor_guia" size="20" value="<%if trim(staguia.Fields.Item("comentarios").Value)<>"0" and trim(staguia.Fields.Item("comentarios").Value)<>"" then %><%=trim(staguia.Fields.Item("comentarios").Value)%><%else%><%=trim(staguia.Fields.Item("comentarios").Value)%><%end if%>">
                         <input name="Submit" type="submit" class="inputbutton" value="Rastreo COORDINADORA">
						 </div>
                         </form>
						 </span>
						 <%end select%>
				...
				<%Select case replace(trim(lcase(staguia.Fields.Item("descri").Value))," ","")
				
				case "despachadoportcc"%>
								<%response.write("&nbsp;<a href=""http://sic.tcc.com.co/cambios/remesas/ver_remesas.asp?Nros=" & (staguia.Fields.Item("comentarios").Value) &  ",-1000"" class=""botones"" target=""_blank"" >" & (staguia.Fields.Item("comentarios").Value) & "</a>")
